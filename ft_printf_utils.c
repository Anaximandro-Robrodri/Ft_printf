...
static const char	*ft_load_prec(const char *str, t_flag *flag, va_list arg)
{
	flag->flag_prec = va_arg(arg, int);
	if (flag->flag_prec > 0)
		flag->flag_is_prec = 1;
	if (flag->flag_prec == 0)
		flag->flag_dot = 1;
...
static void	ft_load_width_astherisk(t_flag *flag, va_list arg)
{
	flag->flag_width = va_arg(arg, int);
...
const char	*ft_check_flags(const char *str, t_flag *flag, va_list arg)
{
	if (*str == '-')
	{
		flag->flag_zero = 0;
	...
	if (*str == '0' && flag->flag_minus == 0)
		...
		str = ft_load_prec(str, flag, arg);
	else if (*str == '*')
		ft_load_width_astherisk(flag, arg);
